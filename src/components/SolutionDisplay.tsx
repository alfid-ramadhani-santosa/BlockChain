...
function StepGrid({ step, size = 32 }: StepGridProps) {
  const {
    ...
    newPieceValue: _npv,
    pieceValue,
  } = step as PlacementStep & { newPieceValue?: number };
  ...
  const isCleared = (r: number, c: number) =>
    clearedRowSet.has(r) || clearedColSet.has(c);
  ...
  return (
    <div
      ...
      style={{ gridTemplateColumns: `repeat(8, ${size}px)`, gap: "2px" }}
    >
          ...
          return (
            <div
              ...
              style={{
                ...
                outline:
                  cleared && cell !== 0 ? "2.5px dashed #f97316" : undefined,
                outlineOffset: "-2px",
...
function SolutionCard({ solution, index, onApply }: SolutionCardProps) {
  ...
  const handleApply = (e: React.MouseEvent) => {
    ...
    const finalBoard = lastStep.boardAfter.map((row) =>
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-6">
          ...
          <div
            className={`text-slate-400 text-lg transition-transform duration-200 ${expanded ? "rotate-180" : ""}`}
          >
            ▾
          </div>
        </div>
      ...
      {expanded && (
        <div className="px-6 py-5" style={{ background: "#f8fafc" }}>
          ...
          <div className="flex gap-4 justify-center mb-4 text-xs text-slate-500">
            ...
            <div className="flex items-center gap-1">
              <div
                className="w-4 h-4 rounded border-2 border-dashed border-orange-400"
            ...
            {solution.steps.map((step, si) => (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    color: step.linesCleared > 0 ? "#f97316" : "#94a3b8",
                  }}
                >
                ...
                <div className="text-xs text-slate-400 mt-2">
                  ...
                  {step.linesCleared > 0 && (
                    <span className="text-orange-500 font-semibold ml-1">
                      +{step.linesCleared} line
                      {step.linesCleared > 1 ? "s" : ""}!
                    </span>
                  )}
                </div>
                ...
                {(step.clearedRows.length > 0 ||
                  ...
                  <div className="text-xs mt-1" style={{ color: "#f97316" }}>
                    {step.clearedRows.length > 0 &&
                      `Row ${step.clearedRows.map((r) => r + 1).join(",")} `}
                    {step.clearedCols.length > 0 &&
                      `Col ${step.clearedCols.map((c) => c + 1).join(",")}`}
                    {" cleared"}
                  </div>
                )}
              </div>
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <div className="text-slate-700 font-semibold text-lg animate-pulse">
          Solving puzzle...
        </div>
        <div className="text-slate-400 text-sm mt-1">
          Trying all piece orderings for maximum score
        </div>
      </div>
    );
  }
  ...
  if (solutions.length === 0) {
    return (
      ...
      >
        ...
        <div className="text-slate-800 font-bold text-xl mb-2">
          No Valid Solution Found
        </div>
        <div className="text-slate-500 text-sm leading-relaxed max-w-md mx-auto">
          A valid solution requires{" "}
          <span className="text-indigo-600 font-semibold">
            all pieces to be placed
          </span>{" "}
          on the board.
        </div>
        <div
          className="mt-5 rounded-xl p-4 text-xs text-left max-w-sm mx-auto"
        ...
        >
          ...
          <div className="text-slate-500">
            ...
            <div>
              • Try <strong>Thorough</strong> optimization mode
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="text-sm text-cyan-400">
          ...
          <span className="font-bold">
            {maxLines} line{maxLines !== 1 ? "s" : ""} cleared
          </span>
        </div>
      </div>
